...
class StudentHallTicket(models.TransientModel):
	...
	exam_session_id = fields.Many2one(
		'student.exam.session', string='Exam Session', required=True,
		domain=[('state', 'not in', ['draft', 'cancel', 'done'])])

	def print_report(self):
		data = self.read(['exam_session_id'])[0]
		return self.env.ref('school_management.action_student_hall_ticket_report').report_action(self, data=data)
